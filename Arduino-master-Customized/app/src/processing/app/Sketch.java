...
//  }
   ...
   */
  public String build(boolean verbose) throws IOException,InterruptedException, RunnerException, PreferencesMapException {
   ...
   * @return null if compilation failed, main class name if not
   */
  public String build(String buildPath, boolean verbose) throws IOException,InterruptedException, RunnerException, PreferencesMapException {
    ...
    // editor.setComponentOrientation(ComponentOrientation.RIGHT_TO_LEFT);
			//if (!editor.getText().startsWith("Arduino") && !editor.getText().startsWith("arduino") ){
	
	
	 if (editor.rtl){
	 
	 String convertedcode= Convertcode(editor.getText().replaceAll("\"", "'"));
	   
        
          
   // String result = JOptionPane.showMessageDialog(editor, null,convertedcode);
      //Base.showMessage("Arduino Bari3",System.getProperty("user.dir") + "\\" + "bari3.exe" );
	  Base.showMessage("Arduino Bari3", convertedcode);
     current.getCode().setProgram(convertedcode);
	 
     data.replaceCode((current.getCode()));
	 editor.statusNotice( _("compile bari3 commands to arduino c complete .."));
     }
...
String Convertcode(String code)   throws IOException, InterruptedException  {

String full_code="";
ProcessBuilder   ps=new ProcessBuilder( System.getProperty("user.dir") + "/" + "bari3",code);
...
full_code=	full_code.concat(line+"\n");
}
in.close();
